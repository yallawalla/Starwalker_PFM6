...
{
int i,*p,*q=NULL;
	for(p=(int *)FATFS_ADDRESS; p[SECTOR_SIZE/4]!=-1 && (int)p < FATFS_ADDRESS+SECTOR_SIZE*SECTOR_COUNT; p=&p[SECTOR_SIZE/4+1])
			...
			do {
				do {
					...
					if(q[SECTOR_SIZE/4] == -1) {
						...
						Watchdog();
						if(mode)
							STORAGE_Write (*FSDRIVE_CPU-'0',(uint8_t *)buf,p[SECTOR_SIZE/4],1);
					} else
						++c1;
					p=&p[SECTOR_SIZE/4+1]; 
